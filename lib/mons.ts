...
const allMons = [...kantoMons, ...johtoMons, ...hoennMons, ...sinnohMons, ...unovaMons]
...
export const mons: Mon[] = allMons.map((name, index) => {
  const dexNo = index + 1;
  return {
    dexNo,
    name,
    spriteFront: `${baseUrl}/shiny/${dexNo}.gif`,
    spriteBack: `${baseUrl}/back/shiny/${dexNo}.gif`,
  };
});
